...
//////////////////// check staff if exist first ////////////////
var checkdeletestaff = async (req, res, next) => {
  let user;
  user = await staff_tbl.findOne({ id: xss(req.params.userid) });
  if (user) {
    res.locals.status = "exist";
    next();
  } else {
...
module.exports = {
  ...
  checkdeletestaff: checkdeletestaff,
};
